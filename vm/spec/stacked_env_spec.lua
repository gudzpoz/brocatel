...
describe("StackedEnv", function()
    ...
    it("with global environment by set_global_scope", function()
        ...
        assert.equals(1, env:get_lua_env()["b"])
        env:set_init(false)
        env:set("b", 1)
    ...
    describe("with stacked environment", function()
        it("in a simple case", function()
            ...
            env:push({ k1 = true }, t1)
            env:push({ k2 = true }, t2)
            assert.equals(1, env:get("k1"))
            assert.is_nil(env:get("k2"))
            env:set("k1", 2)
            env:set("k2", 3)
            assert.equals(2, env:get("k1"))
            assert.equals(2, t1["k1"])
            assert.equals(3, env:get("k2"))
            assert.equals(3, t2["k2"])
            env:push({ k1 = true, k3 = true }, { k3 = 3 })
            assert.is_nil(env:get("k1"))
            assert.equals(3, env:get("k3"))
            env:pop()
        ...
        it("in a normal env", function()
            ...
            env:set("k1", 1)
            env:set("k2", 2)
            assert.equals("ok", t1.k1)
            assert.equals(2, t1.k2)
            assert.equals(1, t2.k1)
        end)
    end)
